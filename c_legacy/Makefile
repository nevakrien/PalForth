# Default: no arena
SHARED_FLAGS := -O2 -g -Wall #-fsanitize=address -fsanitize=undefined
CFLAGS := -std=c99 $(SHARED_FLAGS)
OBJ = header_impels.o vm.o arena.o types.o
CC = clang
CLANG = clang
CLANGXX = clang++
...
test: test.out #compiler_test.out jit_test.out
	./test.out
# 	./compiler_test.out
# 	./jit_test.out
...
compiler_test.out: compiler_test.c $(OBJ) $(HEADERS)
	$(CC) $(CFLAGS) $(OBJ) compiler_test.c -o $@
